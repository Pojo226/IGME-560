...
public class FishTier2 : FishController {
			...
			foreach(Collider nearby in colliders){
				...
				if(fish && fish != this){
					...
					if(offset > 0 && offset <= 2){ //If the other fish is bigger, and not too big...
						Debug.Log("Flee!!!");
						SetDestination(Destination_Types.Flee, transform.position + (Vector3.Normalize(transform.position - nearby.transform.position)) * 4);
						return true;
					}
				}
			}
	...
	protected override void FindShelter(){
		...
		//Panic time.
		SetDestination(Destination_Types.Shelter, GenerateRandomPoint(transform.position, 4));
	}
	...
	protected override void Idle(){
		if(destinationType == Destination_Types.None){
			SetDestination(Destination_Types.Idle, GenerateRandomPoint(transform.position, 4));
		}
	}
}
